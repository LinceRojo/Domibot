...
class MotorPasAPas:
    ...
    def moure_a_graus(self, graus_objectiu: float, velocitat_graus_per_segon: float = 60.0):
        ...
        # Normalitza l'objectiu a [0, 360)
        graus_objectiu = graus_objectiu % 360
        if graus_objectiu < 0:
            graus_objectiu += 360

        # Converteix la posició actual en passos a graus (normalitzada a [0, 360))
        posicio_actual_graus = (self.posicio_actual_passos % self.passos_per_volta) * (360.0 / self.passos_per_volta)
        posicio_actual_graus = posicio_actual_graus % 360
        if posicio_actual_graus < 0:
            posicio_actual_graus += 360

        graus_a_moure = graus_objectiu - posicio_actual_graus

        # Decideix la direcció més curta
        direccio = self.DIRECCIO_ENDAVANT
        if abs(graus_a_moure) > 180:
            # Si el camí més curt és anar per l'altre costat (creuant el 0/360)
            graus_a_moure = (360 - abs(graus_a_moure)) * (-1 if graus_a_moure > 0 else 1)
        ...
        if graus_a_moure < (360.0 / self.passos_per_volta) / 2 and graus_a_moure > 0: # Si la diferència és menor que mig pas
            # Ajustar per garantir un petit moviment si ja estem molt a prop
            passos_per_grau = self.passos_per_volta / 360.0
            if passos_per_grau * graus_a_moure < 1: # Si els graus no arriben ni a 1 pas, fem 1 pas en la direcció correcta
                passos_a_moure = 1
                if graus_objectiu < posicio_actual_graus: # Determineu la direcció correcta si els graus a moure eren negatius
                     direccio = self.DIRECCIO_ENRERE
                else:
                    direccio = self.DIRECCIO_ENDAVANT
        ...
        if graus_a_moure == 0:
            print(f"Motor '{self.nom}': Ja a la posició objectiu de {graus_objectiu:.2f} graus.")
            return

        print(f"Motor '{self.nom}': De {posicio_actual_graus:.2f} graus a {graus_objectiu:.2f} graus. Movent {graus_a_moure:.2f} graus.")
        self.moure_n_graus(graus_a_moure, direccio, velocitat_graus_per_segon)

    def calibrar(self, passos_max_calibratge: int = 4096, direccio_calibratge: bool = DIRECCIO_ENRERE, velocitat_graus_per_segon: float = 60.0):
        """
        ...
        **Nota:** En una aplicació real, aquesta funció hauria de connectar-se
        ...
        Args:
            passos_max_calibratge (int): Nombre màxim de passos per moure's durant el calibratge.
        ...
        print(f"Simulant calibratge: movent {passos_max_calibratge} passos i establint la posició a 0.")
        self.moure_n_passos(passos_max_calibratge, direccio_calibratge, velocitat_graus_per_segon)
    ...
    def obtenir_posicio_graus(self) -> float:
        """
        Retorna la posició actual del motor en graus (entre 0 i 359.99).
        """
        # Normalitza la posició_actual_passos per obtenir un valor positiu i dins del rang.
        pos_passos_normalitzada = self.posicio_actual_passos % self.passos_per_volta
        if pos_passos_normalitzada < 0:
            pos_passos_normalitzada += self.passos_per_volta
        return pos_passos_normalitzada * (360.0 / self.passos_per_volta)
